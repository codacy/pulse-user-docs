...
The Pulse CLI is an easy-to-use abstraction over the Pulse Ingestion API. If the CLI requirements below don't fit your use case, explore the Pulse's [Ingestion API docs ↗](https://ingestion.pulse.codacy.com/v1/api-docs){: target=_"blank"} for interacting directly with the API.
